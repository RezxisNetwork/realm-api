...
public class StatisticsLogging implements Runnable {
	...
	public static HashMap<Date,String> search(String type, Date from) {
		...
		try {
			SearchSourceBuilder builder = new SearchSourceBuilder().fetchSource(new String[] {"@timestamp","value","type"}, new String[0]);//.query(QueryBuilders.rangeQuery("timestamp").from(from).to(new Date()));
